...
/*1)List the students who didn’t appear for any exams.
		Format: name*/
select students.name from marks inner join students on marks.student_id=students.id where annual is null and quarterly is null and half_yearly is null;
...
/*ordered by name*/
select name,SUM(annual) as marks,year from marks inner join students on marks.student_id=students.id group by year,name order by name;

/*ordered by year*/
select name,SUM(annual) as marks,year from marks inner join students on marks.student_id=students.id group by year,name order by year;
...
select name,SUM(quarterly) as total,grade from marks inner join students on marks.student_id=students.id where year=2003 group by student_id,grade;
...
/*6)List the students who has not yet received any medals but scored more than 90 marks in all the subjects in the annual exam for that year.
		Format: name, year*/
select students.name,marks.year from students left join medals on students.id = medals.student_id left join marks on marks.student_id = students.id group by students.id,medals.id,marks.year having sum(if(annual>90,0,1)) = 0 and count(medals.id) = 0;
