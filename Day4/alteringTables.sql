...
select derived1.student_id,derived1.name,derived1.year,derived1.percentage,coalesce(derived2.no_of_medals_received,0)as no_of_medals_received from (select student_id,name,year,((sum(annual)/500)*100) as percentage from marks inner join students on student_id=students.id group by student_id,year)as derived1 left join (select student_id,year,count(medal_won)as no_of_medals_received from medals group by student_id,year) as derived2 on derived1.student_id=derived2.student_id and derived1.year=derived2.year;
